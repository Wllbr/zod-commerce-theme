...
$rootFiles += @('DEPARTMENTS_AR.md', 'AUDIT_REPORT.md', 'REVISION_1_9_10.md')
...
foreach ($kind in @('Salla-Upload', 'Source')) {
  ...
  try {
    ...
    # v1.9.10 retains readable full-scene campaign artwork. The archive includes
    # both source and compiled copies; this local budget is not a Salla limit.
    if ($kind -eq 'Salla-Upload' -and (Get-Item -LiteralPath $destination).Length -gt 1.25MB) { throw "Upload archive exceeds this project's configured 1.25 MB release budget" }

...
$rootFiles += @('DEPARTMENTS_AR.md', 'AUDIT_REPORT.md', 'REVISION_1_9_11.md')
...
foreach ($kind in @('Salla-Upload', 'Source')) {
  ...
  try {
    ...
    # Merchant requires a strict decimal 1 MB upload maximum.
    if ($kind -eq 'Salla-Upload' -and (Get-Item -LiteralPath $destination).Length -gt 1000000) { throw "Upload archive exceeds this project's configured 1,000,000-byte release budget" }

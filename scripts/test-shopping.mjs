...
check('checkout summary is unique and mobile navigation remains available',()=>{assert.equal((cart.match(/<salla-cart-summary-card\b/g)||[]).length,1);assert.doesNotMatch(cart,/store-cart-checkout-mobile|zod-cart-mobile-summary|data-zod-cart-tax/);assert.doesNotMatch(master,/show_mobile_bottom_nav[^\n]*not is_page/);const commerce=read('src/assets/styles/commerce-states.scss');const revision=read('src/assets/styles/revision-1910.scss');assert.match(commerce,/@media\(max-width:1023px\)[\s\S]*salla-cart-summary-card\.zod-cart-summary-card[^}]*bottom:/,'mobile native checkout remains docked');assert.match(revision,/@media\(min-width:1024px\)[\s\S]*\.s-cart-summary-card-action\{position:static!important/,'desktop checkout sits below the native summary');assert.doesNotMatch(revision,/padding-bottom:100px!important/,'desktop no longer reserves space for a fixed checkout');});
...
// is restricted in the local preview sandbox; URL/recovery logic is verified here.
{
  ...
  const nativeError=new Element();nativeError.getClientRects=()=>[];
  list.nativeFilters=nativeFilters;list.querySelectorAll=selector=>selector.includes('s-infinite-scroll-error')?[nativeError]:[];list.reload=()=>{reloads++;return new Promise(r=>resolveReload=r);};
  ...
  vm.runInNewContext(source,{URL,MutationObserver:class {constructor(f){handlers.observe=f;}observe(){}},containDialogFocus:()=>{},document:{getElementById:id=>id==='product-filter'?sort:null,querySelectorAll:()=>[],querySelector:s=>selectors[s]||null,addEventListener:(n,f)=>{if(n==='DOMContentLoaded')ready=f;}},window:{location,history,matchMedia:()=>({matches:false,addEventListener:()=>{}})}});
  ready();assert.equal(recovery.hidden,true,'hidden native error does not show theme retry');
  nativeError.getClientRects=()=>[{}];handlers.observe();assert.equal(recovery.hidden,false,'visible native error shows theme retry');
  const pending=handlers.change();
